...
class ParticipantOfRace:
    ...
    def from_string(cls, index_number, riders_name_point_str):
        ...
        is_kondratiy = set('kondratiy') <= set(name)
...
def sort_riders(riders_list):
    riders1 = list()
    riders2 = list()
    while len(riders_list) > 0:
        item = riders_list.pop()
        if item.is_kondratiy:
            riders1.append(item)
        else:
...
def main(input_str):
    ...
    for i, item in enumerate(input_str[1:]):
        riders_list.append(ParticipantOfRace.from_string(i, item))
    answer_str = ''
    for item in sort_riders(riders_list):
        answer_str += str(item) + '\n'
    return answer_str
